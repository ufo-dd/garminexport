...
export_formats = format_suffix.keys()
...
def download(client, activity, backup_dir, export_formats=None):
	...
	with open(not_found_path, mode="a") as not_found:    
		...
		if 'orig' in export_formats:
			activity_zip = client.get_activity_orig(id)
			dest = os.path.join(
				backup_dir, export_filename(activity, 'orig'))
			if activity_zip is None:
				not_found.write(os.path.basename(dest) + "\n")
			else:
				with open(dest, mode="wb") as f:
					f.write(activity_zip)
